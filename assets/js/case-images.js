/* ============================================================
   ...
   ▶ 옵시디언 노트(발행대기 *.md) 프론트매터에 다음을 적으면
     그대로 우선 적용됩니다.

       representative_image: representative.jpg   # 완성(시공 후) 대표 사진
       before_images:
         - before-01.jpg
       after_images:
         - after-01.jpg
     ...
     · 경로 변환은 빌드 도구(tools/sync-case-images.js)가 담당하고,
       이 파일은 이미 변환된 경로를 받아 역할만 정리합니다.
   ...
   ▶ 결과 형태
     {
       representativeImage : String,   // 카드 썸네일 = 완성/AFTER 사진
       beforeImages        : [String], // 시공 전
       afterImages         : [String], // 시공 후
       galleryImages       : [String], // 위에서 쓰이지 않은 나머지 사진
       hasBefore           : Boolean,
       hasAfter            : Boolean,
       showComparison      : Boolean,  // 상세페이지 BEFORE/AFTER 구간 노출 여부
...
(function (root, factory) {
  ...
  function normalize(caseData) {
    ...
    /* 1. 명시 항목 (옵시디언 프론트매터에서 온 값) */
    var explicitRep    = cleanPath(c.representative_image);
    ...
    /* 5. 나머지 사진 — 대표/전/후 어디에도 쓰이지 않은 것만 */
    var used = {};
    if (representativeImage) used[representativeImage] = true;
    beforeImages.concat(afterImages).forEach(function (p) { used[p] = true; });

    var galleryImages = toList(c.images).filter(function (p) { return !used[p]; });

    /* 6. 상세페이지 BEFORE/AFTER 구간 노출 판단
          - 시공 전 사진이 있으면 항상 노출
          - 시공 후 사진이 여러 장이면 노출
          - 시공 후 사진이 대표 이미지와 다른 사진이면 노출
          (= 대표 이미지 한 장뿐인 예전 사례는 중복 노출하지 않습니다) */
    var hasBefore = beforeImages.length > 0;
    var hasAfter  = afterImages.length > 0;
    var showComparison = hasBefore ||
